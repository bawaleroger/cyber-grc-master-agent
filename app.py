...
st.set_page_config(page_title="Cyber-GRC V8.1 OpenRouter", page_icon="🛡️", layout="wide")
OPENROUTER_KEY = st.secrets.get("OPENROUTER_API_KEY","") or os.getenv("OPENROUTER_API_KEY","")

st.markdown('<h1 style="color:#0B1D3A">🛡️ CYBER-GRC MASTER AGENT V8.1 - OPENROUTER</h1>', unsafe_allow_html=True)
st.caption("OpenRouter uniquement - Modèles gratuits corrigés + RAG + ISO 42001 + XOF")
...
with st.sidebar:
    ...
    st.divider()
    model = st.selectbox("Modèle (100% gratuits vérifiés)", [
...
def call_llm(system_prompt, messages, model_name):
    if not OPENROUTER_KEY:
        return "❌ Ajoute OPENROUTER_API_KEY dans Secrets (Manage app -> Settings -> Secrets)"
    from openai import OpenAI
    fallbacks = [model_name, "meta-llama/llama-3.3-70b-instruct:free", "google/gemini-2.0-flash-exp:free", "deepseek/deepseek-r1:free"]
    last_err=""
    for m in fallbacks:
        try:
            ...
            resp = client.chat.completions.create(
                ...
                temperature=0.2,
                max_tokens=5000,
                extra_headers={"HTTP-Referer": "https://cyber-grc.streamlit.app", "X-Title": "Cyber-GRC V8.1"}
            )
            if m != model_name:
                return resp.choices[0].message.content + f"\n\n_[Fallback auto: {m} car {model_name} indisponible]_"
            return resp.choices[0].message.content
        except Exception as e:
            last_err=str(e)
            if "404" in last_err or "unavailable for free" in last_err:
                continue
            return f"Erreur OpenRouter ({m}): {e}"
    return f"Erreur: {last_err}. Va sur openrouter.ai -> Credits: ajoute 1$ (tu resteras en free) ou choisis llama-3.3-70b:free"

tab1, tab2, tab3 = st.tabs(["📄 Dépôt TDR", "🧠 Chat OpenRouter", "📂 Docs"])
with tab1:
    st.subheader("Dépose ton TDR ici (pas dans le chat !)")
    st.info("Docs = 0 actuellement. Glisse ton PDF ici.")
    uploaded = st.file_uploader("PDF, DOCX, XLSX", accept_multiple_files=True, type=["pdf","docx","xlsx","pptx","txt","md"], key="up_v81")
    if uploaded:
...
with tab2:
    ...
    st.warning("⚠️ Ne dis pas 'je t'envoie une TDR' dans le chat. Va dans Dépôt TDR pour uploader ! Docs actuel = 0")
