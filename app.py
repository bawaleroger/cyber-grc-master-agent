...
with tab3:
    ...
    search = st.text_input("Rechercher dans KB", key="search_kb2")
    if search:
    ...
    for d in docs:
        with st.expander(f"{d['classification']} | {d['filename']}"):
            ...
            path = BASE / d["path"]
            if path.exists():
                st.download_button(f"Télécharger {d['filename']}", data=open(path,"rb").read(), file_name=d['filename'], key=f"dl_{d["id"]}_{d["filename"]}")
