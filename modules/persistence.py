...
# Chemin absolu pour persistance reelle sur Streamlit Cloud
...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS documents 
                 (id TEXT PRIMARY KEY, filename TEXT, type TEXT, norme TEXT, content TEXT, hash TEXT, date_ingest TEXT, client TEXT, is_kb INTEGER DEFAULT 0)""")
    c.execute("""CREATE TABLE IF NOT EXISTS tdr 
                 (id TEXT PRIMARY KEY, client TEXT, cabinet TEXT, objectif TEXT, perimetre TEXT, secteur TEXT, normes TEXT, livrables TEXT, contraintes TEXT, raw_text TEXT, date TEXT)""")
    # Migration: ajoute is_kb si ancienne DB
    try:
        c.execute("SELECT is_kb FROM documents LIMIT 1")
    except:
        try:
            c.execute("ALTER TABLE documents ADD COLUMN is_kb INTEGER DEFAULT 0")
        except: pass
    conn.commit()
    conn.close()
    return str(DB_PATH)
...
def save_document(filename, content, doc_type, norme, client="GEN", is_kb=False):
    ...
    c = conn.cursor()
    # Verifie si existe deja par hash pour eviter doublons
    ...
    c.execute("INSERT OR REPLACE INTO documents VALUES (?,?,?,?,?,?,?,?,?)",
              (doc_id, filename, doc_type, norme, content, content_hash,
               datetime.datetime.now().isoformat(), client, 1 if is_kb else 0))
    conn.commit()
    conn.close()
    # Sauvegarde physique pour double securite
    try:
        safe_name = "".join([c for c in filename if c.isalnum() or c in "._-"])[:50]
        (KB_DIR / f"{doc_id}_{safe_name}.txt").write_text(content[:20000], encoding="utf-8", errors="ignore")
    except Exception as e:
        print(f"Save physical failed {e}")
    return doc_id
...
def get_kb_context(limit_chars=20000):
    db = init_db()
    conn = sqlite3.connect(db)
    c = conn.cursor()
    ...
    texts = c.fetchall()
    conn.close()
    # Si DB vide, tente lecture fichiers physiques
    if not texts:
        try:
            ctx=""
...
def save_tdr(client, cabinet, tdr_dict, raw_text):
    ...
    c.execute("INSERT OR REPLACE INTO tdr VALUES (?,?,?,?,?,?,?,?,?,?,?)",
              (tid, client, cabinet, tdr_dict.get('objectif',''), tdr_dict.get('perimetre',''),
               tdr_dict.get('secteur',''), json.dumps(tdr_dict.get('normes',[])),
               json.dumps(tdr_dict.get('livrables',[])), tdr_dict.get('contraintes',''),
               raw_text, datetime.datetime.now().isoformat()))
    conn.commit()
    conn.close()
    return tid
...
def get_latest_tdr():
    ...
    return {
        ...
        "tdr_dict": {"objectif": objectif, "perimetre": perimetre, "secteur": secteur, "normes": normes, "livrables": livrables, "contraintes": contraintes, "raw_preview": raw_text[:2000]},
        "raw_text": raw_text, "date": date
    }
...
def get_all_tdr():
    ...
    conn = sqlite3.connect(db)
    c = conn.cursor()
    c.execute("SELECT id, client, date, substr(raw_text,1,100) FROM tdr ORDER BY date DESC")
    rows = c.fetchall()
    conn.close()
    return rows
